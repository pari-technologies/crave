...
class MarkerIcon {
  ...
  static Future<BitmapDescriptor> downloadResizePictureCircle(
    ...
    final Path clipPath = Path();
    clipPath
        .addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(0, 0, size.toDouble(), size.toDouble()), Radius.circular(30)));
    ...
    final _image = await pictureRecorder.endRecording().toImage(size, (size * 1).toInt());
    final data = await _image.toByteData(format: ui.ImageByteFormat.png);
